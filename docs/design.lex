...
4. Code Review Observation
    ...
    `ReviewTargetsProvider` is separate from `CodeReviewsProvider`. A provider implements it only when it can inspect review-request addresses, so adding the capability does not add a required method to existing code-review provider implementations. `inspect_review_request_target` accepts one repository and one `ReviewRequestTarget`. Callers inspect several targets by calling it for each target before persisting any of them.

    `ReviewRequestTargetInspection` returns the observed `ReviewActor` or `ReviewTeam` and classifies it as matching the requested user, bot or team category, or as a category mismatch. The constructor applies that classification from the observed kind: users and enterprise users match user targets, bots match bot targets and teams match team targets. Placeholder and organization actors do not match a user or bot target. An unresolved result means the provider did not find the target or the current credentials could not see it; it does not establish that the target does not exist. Authentication failures, malformed inputs and transport failures remain structured errors.

    GitHub inspects a user target at its exact login first, then tries the same login with `[bot]` after a 404. It inspects a bot target at the `[bot]` login first, then tries the unsuffixed login after a 404. A non-404 failure stops the search, and the REST response's `type` supplies the observed actor kind. A team target must have the form `organization/team-slug`; the provider reads that organization team and uses its returned identifier, slug and name. These reads establish identity and category only. They do not establish repository assignability, application installation or delivery; `CodeReviewsProvider::request_reviewers` performs the write and confirms the outstanding request separately.
