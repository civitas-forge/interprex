...
use interprex::{ConfigurationSource, ProviderError, Result};
...
pub async fn from_project(project_root: &Path) -> Result<GithubProvider> {
    let path = project_root.join(".interprex.toml");
...
mod tests {
    ...
    impl TempProject {
        fn new(contents: Option<&str>) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "interprex-github-test-{}-{}",
                std::process::id(),
                NEXT.fetch_add(1, Ordering::Relaxed)
            ));
            std::fs::create_dir(&path).expect("create temporary project");
            if let Some(contents) = contents {
                std::fs::write(path.join(".interprex.toml"), contents)
    ...
    async fn project_app_error_reports_the_file_that_supplied_it() {
        ...
        assert!(error.to_string().contains(&format!(
            "file {}",
            project.path().join(".interprex.toml").display()
        )));
    }
        ...
        assert_eq!(
            ...
            ProviderError::Configuration {
                origin: ConfigurationSource::File(
                    project.path().join(".interprex.toml").display().to_string()
                ),
    ...
    fn unreadable_project_file_returns_a_structured_configuration_error() {
        ...
        assert_eq!(
            ...
            ProviderError::Configuration {
                origin: ConfigurationSource::File(
                    project_root.join(".interprex.toml").display().to_string()
                ),
