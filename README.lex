...
3. Code Review Data
    ...
    Checks are read by commit rather than by change request, and the read returns the current run of each check: a rerun replaces the run it repeated, so each check name appears once. Each check carries its name, that commit, its status, the application that published it, its published summary and where a person can read it. A check that has not finished is pending and has no conclusion; a completed check carries its conclusion and the time it finished. Publishing a check uses a narrower conclusion vocabulary than reading one, because a platform can report conclusions it refuses to accept from a client, such as GitHub's `stale`.
